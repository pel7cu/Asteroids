...
namespace GameDev2D
{
    ...
    {
        float c = cosf(aRadians);
        float s = sinf(aRadians);

        Matrix rotation = Matrix::Identity();
        rotation.m[0][0] = c;  rotation.m[1][0] = -s;
        rotation.m[0][1] = s;  rotation.m[1][1] = c;
        *this = rotation * *this;
    }
	...
	float Matrix::GetRadians() const
	{
		float radians = atan2f(m[0][1], m[1][1]);
		return radians;
	}

	float Matrix::GetDegrees() const
	{
